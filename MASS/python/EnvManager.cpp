...
PYBIND11_MODULE(pymss, m)
{
	py::class_<EnvManager>(m, "pymss")
		...
		.def("SetActions",[](EnvManager& self, py::array_t<double> arr) {
			py::buffer_info buf = arr.request();
			if (buf.ndim != 2) throw std::runtime_error("SetActions: expected 2D array");
		...
		.def("SetActivationLevels",[](EnvManager& self, py::array_t<double> arr) {
			py::buffer_info buf = arr.request();
			if (buf.ndim != 2) throw std::runtime_error("SetActivationLevels: expected 2D array");
			int rows = buf.shape[0];
			int cols = buf.shape[1];
			Eigen::MatrixXd mat(rows, cols);
			double* ptr = static_cast<double*>(buf.ptr);
		...
		.def("SetExoTorques", [](EnvManager& self, py::array_t<double> arr) {
			py::buffer_info buf = arr.request();
			if (buf.ndim != 2) throw std::runtime_error("SetExoTorques: expected 2D array");
			int rows = buf.shape[0];
			int cols = buf.shape[1];
			Eigen::MatrixXd mat(rows, cols);
			double* ptr = static_cast<double*>(buf.ptr);
